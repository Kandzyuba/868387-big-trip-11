...
const getTripInterval = (points) => {
  if (points.length === 0) {
    ...
    return new Date(points[0].startPointDate).toDateString().substr(4, 6);
  } else {
    return new Date(points[0].startPointDate).toDateString().substr(4, 6) + ` - ` + (new Date(points[points.length - 1].endPointDate)).toDateString().substr(4, 6);
  }
};
...
export default class Info extends AbstractComponent {
  ...
  getTemplate() {
    return createInfoTemplate(this._pointsModel.getProcessedPoints());
  }
}
